...
public final class Queue {
	...
	public Queue(final MongoCollection<Document> mongoCollection) {
	 ...
	 *
	 * @param beforeSort
	 *            fields in get() call that should be before the sort fields in
	 *            the index. Should not be null
	 */
	 ...
	 *            the index. Should not be null
	 * @param afterSort
	 *            fields in get() call that should be after the sort fields in
	 *            the index. Should not be null
	 */
	public void ensureGetIndex(final BasicDBObject beforeSort, final BasicDBObject afterSort) {
		...
		// using general rule: equality, sort, range or more equality tests in
		// that order for index
		...
		ensureIndex(new BasicDBObject("running", 1).append("resetTimestamp", 1));// for
																					// the
																					// stuck
																					// messages
																					// query
	 ...
	 *
	 * @param index
	 *            fields in count() call. Should not be null
	 * @param includeRunning
	 *            whether running was given to count() or not
	 */
	 ...
	 *
	 * @param query
	 *            query where top level fields do not contain operators. Lower
	 *            level fields can however. eg: valid {a: {$gt: 1}, "b.c": 3},
	 *            invalid {$and: [{...}, {...}]}. Should not be null.
	 * @param resetDuration
	 *            duration in seconds before this message is considered
	 *            abandoned and will be given with another call to get()
	 ...
	 *            abandoned and will be given with another call to get()
	 * @param waitDuration
	 *            duration in milliseconds to keep polling before returning null
	 * @return message or null
	 */
	 ...
	 *            duration in milliseconds to keep polling before returning null
	 * @param pollDuration
	 *            duration in milliseconds between poll attempts
	 * @return message or null
	 */
	public BasicDBObject get(final BasicDBObject query, final int resetDuration, final int waitDuration,
			long pollDuration) {
		...
		collection.updateOne(
				new BasicDBObject("running", true).append("resetTimestamp", new BasicDBObject("$lte", new Date())),
				new BasicDBObject("$set", new BasicDBObject("running", false)), updateOptions);
		...
		final BasicDBObject update = new BasicDBObject("$set",
				new BasicDBObject("running", true).append("resetTimestamp", resetTimestamp));
		...
		while (true) {
			...
			final Document message = collection.findOneAndUpdate(builtQuery, update, findOneAndUpdateOptions);
			if (message != null) {
	 ...
	 *            invalid {$and: [{...}, {...}]}. Should not be null
	 * @param running
	 *            count running messages or not running
	 * @return count
	 */
	 ...
	 *
	 * @param message
	 *            message received from get(). Should not be null.
	 */
	 ...
	 *
	 * @param message
	 *            message to ack received from get(). Should not be null
	 * @param payload
	 *            payload to send. Should not be null
	 */
	 ...
	 *            payload to send. Should not be null
	 * @param earliestGet
	 *            earliest instant that a call to get() can return message.
	 *            Should not be null
	 */
	 ...
	 *            Should not be null
	 * @param priority
	 *            priority for order out of get(). 0 is higher priority than 1.
	 *            Should not be NaN
	 */
	public void ackSend(final BasicDBObject message, final BasicDBObject payload, final Date earliestGet,
		...
		final BasicDBObject newMessage = new BasicDBObject("payload", payload).append("running", false)
				.append("resetTimestamp", new Date(Long.MAX_VALUE)).append("earliestGet", earliestGet)
				.append("priority", priority).append("created", new Date());

		// using upsert because if no documents found then the doc was removed
		// (SHOULD ONLY HAPPEN BY SOMEONE MANUALLY) so we can just send
		UpdateOptions updateOptions = new UpdateOptions();
		updateOptions.upsert(true);
		updateOptions.bypassDocumentValidation(false);
		collection.updateOne(new BasicDBObject("_id", id), newMessage, updateOptions);
	}
	 ...
	 *
	 * @param message
	 *            message to requeue received from get(). Should not be null
	 */
	...
	public void send(final BasicDBObject payload, final Date earliestGet, final double priority) {
		...
		final Document message = new Document("payload", payload).append("running", false)
				.append("resetTimestamp", new Date(Long.MAX_VALUE)).append("earliestGet", earliestGet)
				.append("priority", priority).append("created", new Date());
	...
	private void ensureIndex(final Bson index) {
		for (int i = 0; i < 5; ++i) {
			for (String name = UUID.randomUUID().toString(); name.length() > 0; name = name.substring(0,
					name.length() - 1)) {
				// creating an index with the same name and different spec does
				// nothing.
				// creating an index with different name and same spec does
				// nothing.
				// so we use any generated name, and then find the right spec
				// after we have called, and just go with that name.
				...
				for (Document document : collection.listIndexes()) {
					if (document.get("key").equals(index)) {
						return;
					}
				}
